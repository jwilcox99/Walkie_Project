...
void handlePTT(WalkieState &state) {
  ...
  // Act only when the button changes state.
  if (ptt != lastPtt) {
    lastPtt = ptt;
    ...
    if (ptt) {
      ...
      if (state.config.mode == MODE_REMOTE ||
          state.config.mode == MODE_AUTO) {
        remoteRelayPttStart(state);
        remoteRelaySendBinaryTest(state);
      }
    } else {
      ...
      if (state.config.mode == MODE_REMOTE ||
          state.config.mode == MODE_AUTO) {
        remoteRelayPttStop(state);
      }
    }
  }

  // Keep existing LOCAL-mode audio behavior unchanged.
  if (state.config.mode != MODE_LOCAL) {
